...
export async function createFileVersionRecord(client: PrismaClient, fileVersion: CreateFileVersionInput): Promise<FileVersion & { url: string }> {
    ...
    if (bucket) {
        ...
        throw new Error("Could not instabtiate file bucket")
    }
}
export async function getFileVersion(client: PrismaClient, id: FileVersion["id"] ):Promise<FileVersion | null>{
...
export async function renameFileVerion(client: PrismaClient, id: FileVersion["id"], name: FileVersion["name"]):Promise<FileVersion> {
