...
// /Users/ByronDunkley/Documents/projects/file-manager-api-main/
...
// create functions that mimic s3 fn

function getSignedUrl(operation: "get" | "put", key: string) {
    const signed = JSON.stringify({
        operation, key, expires: DateTime.local().plus(SIGNED_URL_EXPIRES).toMillis()
    });
...
export async function downloadLocalFile(signedUrl: string ): Promise<FakeAwsFile> {
    const key = validateSignUrl("get", signedUrl);
...
async function headObject(key: string): Promise<FakeAwsFile> {
    ...
    const info = {
        ...parsedInfo,
        ...(parsedInfo.LastModified ? { LastModified: new Date(parsedInfo.LastModified)} : {})
    }

    return info;
}

export async function uploadLocalFile(signedUrl: string, file: FakeAwsFile):Promise<void>{
    const key = validateSignUrl("put", signedUrl);
    await saveFile(key, {
...
function validateSignUrl(operation: "get" | "put", url: string ):string {
