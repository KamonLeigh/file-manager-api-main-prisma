...
import { getSignedUrl as getSignalS3Url } from '@aws-sdk/s3-request-presigner';
...
function getSignedUrl(operation: "get" | "put", key: string, bucketId:string) {
    const Commend = operation === 'get' ? GetObjectCommand : PutObjectCommand
    return getSignalS3Url(
        s3,
        new Commend({ Bucket: bucketId, Key: key}),
        { expiresIn: SIGNED_URL_EXPIRES.as("seconds") }
    )
}

async function uploadFile(key: string, file: FakeAwsFile, bucketId: string):Promise<string> {
    const { Body, ..._} = file;
    await s3.send(
